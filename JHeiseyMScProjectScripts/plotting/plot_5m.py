...
aapl = pd.read_csv("AAPL_jumps_5min.csv", header=None)
goog = pd.read_csv("GOOG_jumps_5min.csv", header=None)
fb = pd.read_csv("FB_jumps_5min.csv", header=None)
intc = pd.read_csv("INTC_jumps_5min.csv", header=None)
msft = pd.read_csv("MSFT_jumps_5min.csv", header=None)
...
print("\% pos:", float(len(intc_L[intc_L > 0]))/(len(all_L_neg) + len(all_L_pos)))
print("\% neg:", float(len(intc_L[intc_L < 0]))/(len(all_L_neg) + len(all_L_pos)))



plt.hist([aapl_days.values, goog_days.values, fb_days.values, intc_days.values, msft_days.values], bins=252, stacked=True, normed = True)
plt.show()
...
plt.xlabel(r"Barndorff-Nielsen Jump Statistic Magnitude (5 min)")
...
plt.title("Jump Distribution (Pos/Neg) for Individual Stocks (5min)")
